...
class HrPayslip(models.Model):
    ...
    total_deduction = fields.Float(string='Total Deduction', compute='_compute_deductions', store=True)
    payable_days = fields.Float(string='Payable Days', compute='_compute_deductions', store=True,default=0.0)
            ...
            if rec.line_ids and rec.contract_id and rec.employee_id:
                if rec.line_ids and rec.contract_id and rec.employee_id:
                    if rec.line_ids.filtered(lambda x: x.payable_days > 0.0).search([], limit=1):
                        for i in rec.line_ids.filtered(lambda x: x.payable_days > 0.0):
                            rec.payable_days = i.payable_days
                        break;
            else:
                        rec.payable_days = 0.0
