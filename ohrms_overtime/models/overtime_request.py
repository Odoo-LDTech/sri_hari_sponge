...
class HrOverTime(models.Model):
    ...
    shift_name = fields.Many2one('resource.calendar', string='Shift Name', related='employee_id.resource_calendar_id',
                                 readonly=True)
    # shift_in_time = fields.Char(string='Overtime Amount')
            ...
            for recd in self.overtime_type_id.rule_line_ids:
                if recd.from_hrs < self.days_no_tmp <= recd.to_hrs and self.contract_id:
                    if self.contract_id.over_day:
                        cash_amount = self.contract_id.over_day * (recd.hrs_amount * self.days_no_tmp)
...
#     _inherit = 'employee.attendance.report'
#
#     shift_in_time = fields.Char(string='shift_in_time', readonly=True)
