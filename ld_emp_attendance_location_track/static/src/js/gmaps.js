odoo.define('ld_emp_attendance_location_track.gmaps', function (require) {
    "use strict";
        ...
        FormController.include({
            ...
            _onLoadMap: function (event) {
                var self = this;
                console.log('valueeeeeeee',event.data.record);
                console.log('hhhhhhhhhhhhhhhhhhhh',event.data.record.data);
                console.log('valueeeeebgejhebgfjeee',event.data.record.model);
                if(event.data.record){
                    if(event.data.record.model == 'odoo.map') {
                        if(event.data.record.data) {
                            ...
                            console.log('mapkey vaule---->>>>>>',mapkey);
                            if(mapkey != ''){
                                ...
                                ajax.jsonRpc('/mylocation/address','call', {id : id, emp : emp, date : date}).then(function(data){
                                    if (data){
                                      ...
                                      alert(_t("No Datacccccccccccccccccccc Found."));
                                        alert(_t("No Data Found."));
                                    }
                                });
                            }
                            else{
                              alert(_t("No Databbbbbbbbbb Found."));
                                alert(_t("No Data Found."));
                            }
                        }
                    }
  ...
  alert(_t("No Dataaaaaaaaaaaaaaaa Found."));
                    alert(_t("No Data Found."));
                ...
                ajax.jsonRpc('/myattendance','call', {emp : emp, date : date}).then(function(data){
                    if(jQuery.isEmptyObject(data[0])) {
                        alert(_t("Something Went Wrong. Refresh and Try again!!."));
                          alert(_t("No view_id Found.",data[0]));
                    }else{
                        var view_id = false;
                        if (data[1]){
                            view_id = data[1]
                            alert(_t("No view_id Found.",view_id));
                        }
